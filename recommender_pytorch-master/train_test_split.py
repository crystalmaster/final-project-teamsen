...
INPUT_PATH = 'Data/sendo.data'

OUTPUT_PATH_TRAIN = 'Data/sendo.data.train.rating'
OUTPUT_PATH_TEST = 'Data/sendo.data.test.rating'
USER_FIELD = 'user_id'
...
def get_train_test_df(transactions):
    ...
    transactions.sort_values(by = ['time'], inplace = True)
    ...
    train_df.sort_values(by=["user_id", 'time'], inplace = True)
...
def main():
    ...
    transactions = pd.read_csv(INPUT_PATH, sep="\t", names = ['user_id', 'item_id', 'rating', 'time'], engine = 'python')
